...
| トピック | 型 | 説明 |
|---|---|---|
| `/lidar_odometry/odom` | `geometry_msgs/msg/PoseWithCovarianceStamped` | ワールド座標系での累積姿勢（車両座標系基準） |
| `/lidar_odometry/trans_pose` | `geometry_msgs/msg/PoseWithCovarianceStamped` | 1時刻前からの相対位置変換（車両座標系基準） |
...
[8] 共分散推定（後述）
    ...
    ▼
[9] 車両座標系への変換（後述）
    cumulative_vehicle = T_v_l × cumulative_pose × T_v_l⁻¹
    delta_vehicle = T_v_l × delta × T_v_l⁻¹
    │
    ▼
[10] Publish
    ・odom: cumulative_vehicle + 共分散
    ・trans_pose: delta_vehicle + 共分散
    │
    ▼
[11] 状態更新
     prev_cloud = 現フレーム点群
     prev_delta = delta（LiDAR座標系のまま保持）
```
...
NDTマッチングはLiDAR座標系で行われるが、出力は車両座標系で行う必要がある。起動時に読み込むLiDAR→車両の外部キャリブレーションパラメータ（6DoF: x, y, z, roll, pitch, yaw）から変換行列 `T_v_l` を構築し、共役変換によりLiDAR座標系の結果を車両座標系に変換する。

```
T_v_l = Translation(x, y, z) × Rz(yaw) × Ry(pitch) × Rx(roll)

delta_vehicle      = T_v_l × delta_lidar      × T_v_l⁻¹
cumulative_vehicle = T_v_l × cumulative_lidar  × T_v_l⁻¹
```

**設計上のポイント:**
- NDT内部の状態（`prev_delta_`, `cumulative_pose_`）はLiDAR座標系のまま保持。NDTの初期推定値（定速モデル）として正しく機能させるため
- `T_v_l⁻¹` は起動時に1回だけ計算しキャッシュ（`t_lidar_vehicle_`）
- パラメータがすべて0の場合、`T_v_l` は単位行列となり従来と同等の動作

#### 外部キャリブレーションパラメータ

| パラメータ | デフォルト | 説明 |
|---|---|---|
| `lidar_to_vehicle_x` | 0.0 | LiDAR原点の車両座標系X位置 [m] |
| `lidar_to_vehicle_y` | 0.0 | LiDAR原点の車両座標系Y位置 [m] |
| `lidar_to_vehicle_z` | 0.0 | LiDAR原点の車両座標系Z位置 [m] |
| `lidar_to_vehicle_roll` | 0.0 | LiDAR→車両のX軸回転 [rad] |
| `lidar_to_vehicle_pitch` | 0.0 | LiDAR→車両のY軸回転 [rad] |
| `lidar_to_vehicle_yaw` | 0.0 | LiDAR→車両のZ軸回転 [rad] |
